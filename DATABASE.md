...
| Aspect | Design |
|---|---|
| **Purpose** | Prescription dispensing: the pharmacist verifies the prescription (drafted → active) and dispenses it (active → dispensed). The LINE is the unit of dispensing — `prescription_lines` carries the per-line stamps `dispensed_by_staff_id`, `dispensed_at` (plus `status` ordered → dispensed), and the header carries the verification stamps `verified_by_staff_id`, `verified_at`. Returns/reversals are implemented (Phase 3 slice 8): a pharmacist reverses a dispensed line — reason captured, stock restored, reversal recorded in `pharmacy_returns`, refund path opened. A standalone `dispensings` table with batch linkage remains a later-phase plan (batches/expiry not implemented). |
...
| **Important fields** | Lines: `status` (ordered → dispensed → reversed, cancelled), `dispensed_by_staff_id NULL`, `dispensed_at NULL`. Header: `status` (drafted → active → dispensed; discontinued/expired terminal), `verified_by_staff_id NULL`, `verified_at NULL`, `lock_version`. Returns: `pharmacy_returns` (`prescription_line_id`, `prescription_id`, `charge_id`, `quantity_minor > 0 CHECK`, `reason_code` CHECK patient_return/wrong_medication/adverse_reaction/dispensing_error/duplicate_dispense/other, `reason_note NULL` — free text that MAY contain PHI and never reaches audit, `returned_by NULL`, `returned_at`) |
