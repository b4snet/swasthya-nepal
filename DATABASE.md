...
| Aspect | Design |
|---|---|
| **Purpose** | Platform-global permission catalog, namespaced `domain:action` (e.g., `pharmacy:dispense`, `billing:void`). |
| **Primary key** | `id uuid` |
...
> **Implemented with Phase 3 slice 6.** The inpatient stay: admission from an OPEN encounter claims a live AVAILABLE bed atomically (compare-and-swap on `beds (status, current_admission_id, lock_version)` — two clerks can never book the same bed; the partial unique `uq_beds_tenant_current_admission` is the DB backstop), and discharge writes a SIGNED discharge-summary clinical note (type `discharge`) that releases the bed (occupied → cleaning, never immediately reassignable). Every status transition is CAS-guarded (`status, lock_version`); one open admission per patient and per encounter is enforced by partial uniques. The tenant-safe composite FK `fk_beds_tenant_current_admission` (`beds` → `admissions` via `(tenant_id, id)`) was added with the table. **Discharge and transfer are high-risk clinical actions (CLINICAL_SAFETY §16): both require the actor's in-flow identity confirmation (name + MRN on-screen, `identityConfirmed` required + accepted at the request layer — the same discipline as MAR administration) — an incomplete discharge/transfer (missing or false confirmation) is rejected 422 with zero side effects; the confirmation is a client-asserted gate and never enters audit payloads (audit stays facts-only: ids, actor, timestamps).**
>
...
| Aspect | Design |
|---|---|
| **Purpose** | Prescription dispensing: the pharmacist verifies the prescription (drafted → active) and dispenses it (active → dispensed). The LINE is the unit of dispensing — `prescription_lines` carries the per-line stamps `dispensed_by_staff_id`, `dispensed_at` (plus `status` ordered → dispensed), and the header carries the verification stamps `verified_by_staff_id`, `verified_at`. Dispensing is batch-selected (Phase 3 slice 17): the pharmacist either names the exact `stock_batches` row (`batchSelections`) or the system auto-picks FEFO among available, unexpired batches; an EXPIRED batch is never issuable (CAS expiry guard → 409). Each dispensed line records its source batch (`batch_id`, `batch_number`, `batch_expires_at`, `batch_quantity_minor`), so a return restores stock to the SAME batch. Controlled substances require dual verification (Phase 3 slice 17 / Phase 2 policy): a SECOND pharmacist's stamp (`dual_verified_by_staff_id` ≠ dispenser, `dual_verified_at`) before the line is released. Returns/reversals are implemented (Phase 3 slice 8) and support PARTIAL quantities (Phase 3 partial-returns slice): a pharmacist returns part or all of a dispensed line — reason captured, stock restored to the original batch for exactly the returned quantity, one reversal row per return event in `pharmacy_returns`, and one refund request opened per return for exactly `unit price × returned quantity`. The line itself tracks the already-returned quantity (`returned_quantity_minor`) and stays `dispensed` until the FULL dispensed quantity is back (then it flips to `reversed`); multiple partial returns accumulate until fully returned. **STANDALONE dispensing** (PRODUCT_REQUIREMENTS §6.7 `dispensing` entity; the documented remaining-scope `dispensings` table): a pharmacist dispenses a medication DIRECTLY to a patient with NO prescription (`POST /dispensings`, `pharmacy:dispense`) — the same batch/shelf CAS stock machinery and the same posted-charge money math (source_type `dispensing`, linked via `charges.dispensing_id`), one immutable `dispensings` record per dispense (patient, medication, exact batch id + snapshot, quantity, dispenser stamp), and the ledger movement references the record (`inventory_movements.dispensing_id`; `prescription_line_id` NULL). No second stock truth: the batch/shelf CAS and the append-only ledger are shared with prescription dispensing. A dual-required controlled batch is REFUSED at standalone dispensing (409) — the dual-verification surface exists only on prescription lines, so a standalone dispense would issue an unverified controlled substance. Each return also creates ONE in-app `billing` notification (`notifications`, §3.37) typed to the opened refund request — the documented "automatic notification to billing on return" — in the SAME atomic transaction as the return (partial unique `(tenant_id, refund_request_id)`: one notification per refund request, never duplicated by retries or concurrent triggers); no money moves at the return — approval stays the billing approver's separate, segregation-of-duties-gated action. |
...
| **Important fields** | Lines: `status` (ordered → dispensed → reversed, cancelled), `dispensed_by_staff_id NULL`, `dispensed_at NULL`, `returned_quantity_minor bigint default 0` (how much of the dispensed quantity has been returned — `CHECK (returned_quantity_minor >= 0 and returned_quantity_minor <= quantity_minor)`, the over-return database backstop), batch stamps (`batch_id`, `batch_number`, `batch_expires_at date NULL`, `batch_quantity_minor NULL CHECK > 0`), dual-verification stamps (`dual_verified_by_staff_id NULL`, `dual_verified_at NULL`; `CHECK (dual_verified_at is null or dual_verified_by_staff_id is not null)`). Header: `status` (drafted → active → dispensed; discontinued/expired terminal), `verified_by_staff_id NULL`, `verified_at NULL`, `lock_version`. Returns: `pharmacy_returns` (`prescription_line_id`, `prescription_id`, `charge_id`, `quantity_minor > 0 CHECK`, `reason_code` CHECK patient_return/wrong_medication/adverse_reaction/dispensing_error/duplicate_dispense/other, `reason_note NULL` — free text that MAY contain PHI and never reaches audit, `returned_by NULL`, `returned_at`) |
| **Relationships** | N–1 `prescription_lines` (dispense movements); charges `source_type='prescription'` posted per dispensed line (price × quantity, minor units) and linked to the line via `charges.prescription_line_id` (the return's financial trace); `pharmacy_returns` N–1 `prescription_lines` / `prescriptions` / `charges` (composite FKs); `dispensings` N–1 `patients` / `medications` / `inventory_items` / `stock_batches` / `staff` (the standalone record) with its posted charge linked via `charges.dispensing_id` and its ledger movement via `inventory_movements.dispensing_id` |
| **Uniqueness** | Header CAS on `(status, lock_version)`; line `(tenant_id, prescription_id, line_no)` unique; returns are PARTIAL-capable — a line may have MANY `pharmacy_returns` rows (non-unique `(tenant_id, prescription_line_id)` support index), and over-return is impossible: the line row is locked FOR UPDATE at return time, the advance is a CAS on `(status, returned_quantity_minor)` (a stale concurrent returner affects zero rows → 409), and the `returned_quantity_minor <= quantity_minor` CHECK is the database backstop. A line flips dispensed → reversed only when `returned_quantity_minor` reaches the full dispensed quantity; each partial return restores stock and opens a refund request for exactly its own quantity |
...
| Aspect | Design |
|---|---|
| **Purpose** | Implemented lean stock model: one `inventory_items` row per (tenant, facility, medication) with a live on-hand quantity, plus the append-only `inventory_movements` ledger (receipt / adjustment / dispense / return) and the batch/expiry layer (`stock_batches`, Phase 3 slice 17): each received lot tracks `expiry_date`, received/remaining quantity, status (available / depleted / quarantined), and the controlled-substance policy flag. Every dispense/return movement records the exact batch it touched (`stock_batches` row), giving batch-level ledger traceability on top of the shelf-level ledger. Batch/expiry VISIBILITY (ROADMAP Phase 12 acceptance "expiring/expired batches visible and never issuable"): `GET /inventory-items/{item}/batches` returns every batch of an item — available, depleted, quarantined, expired — ordered soonest expiry first, with a date-derived `expiryStatus` (`valid` / `expiring_soon` / `expired`, 90-day presentation window), `daysToExpiry` (negative when expired), per-batch stock, and the controlled flag; the endpoint is read-only (no mutation, no audit, `pharmacy:view`), and the dispensing CAS remains the hard expiry gate — visibility never permits issuing an expired batch. |
...
> **Implemented with Phase 6/7 (charges + invoices), Phase 3 slice 5 (refund/adjustment requests), Phase 3 slice 11 (the completed/disbursement state), Phase 3 slice 18 (deposits + insurance claims), and Phase 13 (charge/invoice void).** Posted charges are immutable; a charge is invoiced at most once (partial unique on `invoice_lines.charge_id` — re-invoicing returns 409, never a double-bill). Invoice lines are frozen snapshots. Refunds/adjustments are REQUEST → APPROVAL → COMPLETION → immutable reversing entry: the approved `refund_requests` row IS the reversal — the original charge is never mutated. Deposits (slice 18) are advance payments held on a patient account and allocated against invoices EXACTLY (CAS on `(status, remaining_minor, lock_version)` — never more than the remaining balance, one allocation per deposit+invoice pair). Amounts are integer minor units end to end.
>
> **Void (implemented, Phase 13):** void is a status with a REQUIRED reason and the approver — never a delete, never an UPDATE of the financial fields. `POST charges/{charge}/void` voids a posted charge that has not been invoiced and has no pending/approved refund reserved (status CAS — a duplicate/concurrent void resolves to exactly one winner, 409 for the loser); an invoiced charge is refused (void the invoice instead). `POST invoices/{invoice}/void` voids a draft/issued invoice with NO payments, deposit allocations, or insurance claims, and CASCADES the void to the charges it was built from (one atomic transaction, same reason + approver); invoices with money or value attached are refused — the refund/credit path is the correction. Both endpoints require `billing:void` (restricted to the financial approver tier — org admin, org finance, hospital admin; the billing clerk who charges can never void: segregation of duties). Void reasons are free text that may contain PHI and are recorded on the row but never echoed in API responses or audit payloads; void audit events carry financial facts only (`charge.voided`, `invoice.voided`).
>
...
| **Audit** | Every charge, invoice issue, payment allocation, deposit, refund, void, adjustment — financial events are immutable once posted; corrections are reversing entries (`MASTER_RULES.md` §37.3). Refund events: `refund.requested`, `refund.approved`, `refund.rejected`, `refund.notification_created` (the pharmacy-return billing alert — facts only: refundRequestId/chargeId/amountMinor/reasonCode/channel) — never reason_note text, never patient names |
...
> **Partially implemented with Phase 3 slice 10 (in-app reminders) and the pharmacy return → billing notification slice (in-app `billing` alerts) — in-app channel only.** The `notifications` table exists exactly as designed below (TENANT tier, RLS enabled + FORCED); the workflows are (1) follow-up reminders: one `appointment_reminder` notification per follow-up plan (typed `follow_up_id` FK + partial unique one-per-plan), and (2) pharmacy returns: one `billing` notification per opened refund request (typed `refund_request_id` FK + partial unique one-per-request), created atomically with the return and surfaced to billing via `GET /refund-requests/{refundRequest}/notification`. Email/SMS/push channels, `notification_templates`, `delivery_attempts`, `notification_preference`, template-driven dispatch, and provider integrations remain the documented later-phase surface (`PRODUCT_REQUIREMENTS` §5.4) — nothing is sent outside the app yet, and no fake channels exist.
...
### 3.57 CDSS and Governed AI (cdss_rules, patient_allergies, cdss_check_results, ai_features, ai_drafts)

Phase 21 (ROADMAP Phase 21, CLINICAL_SAFETY.md §6, §9, AI_RULES.md §1–§19): knowledge-base-driven decision support plus the governed assistive-AI surface. Five tables.

| Field | Detail |
|---|---|
| **Purpose** | `cdss_rules` — the VERSIONED, clinically reviewed knowledge base (`rule_type`: interaction \| allergen \| dose \| pathway; per-type payload in `spec` jsonb, shape-validated by CdssService). `patient_allergies` — documented allergies (`allergen_class` is what the KB keys on). `cdss_check_results` — persisted check alerts (one row per raised alert; `open → overridden` records the prescriber's mandatory reason). `ai_features` — the AI REGISTRY (one row per function per facility; tier, owner, pinned model id/version, purpose/non-goals, min inputs, output schema, confidence threshold, fallback mode, `enabled` KILL SWITCH off-by-default, `model_approved` + `evaluation_ref` evidence gate, review cadence, audit class). `ai_drafts` — grounded assistive outputs pinned to model id/version and their record `source_refs`; `draft → signed` (the clinician's accountable act) or `→ withdrawn`; a draft never mutates a clinical record on its own. |
| **Primary key** | `id` (uuid) on all five |
| **Tier** | **TENANT_FACILITY**. RLS enabled + FORCED (2026_08_17_330200). Policy count added: 5 × 4 = 20 (488 → 508); scoped matrix 123 → 128. |
| **Important fields** | cdss_rules: `rule_type`, `code` (unique per type per facility per version), `severity` (contraindicated/major/moderate/minor; null for pathway), `spec` (jsonb: interaction → medication_a_id/medication_b_id; allergen → medication_id/allergen_class; dose → medication_id/min_daily_mg/max_daily_mg; pathway → condition/suggestion), `version` (pinned — a rule is NEVER edited in place), `status` (draft/active/superseded), `lock_version`. patient_allergies: `patient_id`, `allergen`, `allergen_class`, `severity`, `reaction`, `status` (active/resolved; partial-unique one ACTIVE per class), `recorded_by`. cdss_check_results: `patient_id`, `alert_type`, `rule_code` + `rule_version` (the pinned rule), `severity`, `message` (clinical display only — never in audit), `triggering_facts` (ids/classes — no free-text PHI), `status`, `override_reason`/`overridden_by`/`overridden_at`, `lock_version`. ai_features: `function` (unique per facility), `tier` (1–4 — tier 5 is prohibited, never registered), `model_id`/`model_version` (pinned per output), `min_inputs` (the privilege boundary — only these fields may reach the inference boundary), `output_schema`, `confidence_threshold` (calibrated; below → refuse), `fallback_mode`, `enabled` (kill switch), `model_approved`/`evaluation_ref` (evidence precedes any transmission), `review_cadence`, `audit_class`, `status` (registered/active/retired), `lock_version`. ai_drafts: `function`/`tier`/`model_id`/`model_version` (pinned provenance), `source_refs` (jsonb refs the output is grounded in — AI_RULES.md §2, §15), `output` (the draft/summary), `confidence`, `status`, `signer_staff_id`/`signed_at` (the human act), `correlation_id`. |
| **Governance gates** | Activation requires `evaluation_ref` + `model_approved` (evaluation precedes deployment, AI_RULES.md §12); the kill switch (`enabled`, false by default) must be toggled on explicitly and is audited; invocation degrades LOUDLY (200 + `meta.degraded`, or 503 for draft creation) when not active / disabled / model not approved / inference unavailable / below calibrated confidence — care is NEVER blocked (AI_RULES.md §17); the inference boundary (`AiInferenceGateway`) transmits ONLY to `config('ai.approved_models')` endpoints (AI_APPROVED_MODEL_ENDPOINTS env, HTTPS required, model_id+version must match) — with no allowlist configured, nothing ever leaves the platform (AI_RULES.md §14). |
| **Fail-open** | `CdssService::checkPrescription` catches KB unavailability and returns `degraded: true` (surfaced in the envelope meta and audited as `cdss_check.degraded`) — a check that cannot run is a loud "check unavailable", never a silent pass and never a block (CLINICAL_SAFETY.md §6). |
| **Concurrency** | CAS `lock_version` on every transition: rule draft→active (superseding the prior active — the DB partial-unique backstop enforces exactly one ACTIVE version per code), kill-switch toggle, check-result open→overridden, draft sign/withdraw — exactly one winner under contention. |
| **Isolation** | TENANT_FACILITY, app-layer scoping on every surface (read 404 / write 403 out of scope); composite FKs `(tenant_id, facility_id, id)` back every reference; the privilege boundary is enforced by `min_inputs` (only registry-permitted fields ever reach the inference boundary). |
| **Authentication** | `authorize:cdss:view` (doctor/nurse/pharmacist/admins — run checks, view KB), `authorize:cdss:manage` (admins — curate KB, record overrides), `authorize:ai:view` (clinical + admins), `authorize:ai:manage` (admins — registry, activation, kill switches), `authorize:ai:invoke` (doctor/nurse — invoke features, create drafts), `authorize:ai:sign` (doctor ONLY — the accountable clinician sign-off; nurses can invoke but never sign). |
| **Indexes** | cdss_rules: `uq_cdss_rules_tenant_facility_type_code_version`, `uq_cdss_rules_tenant_facility_active` (partial — one ACTIVE per code), `idx_cdss_rules_tenant_facility_type`. patient_allergies: `uq_patient_allergies_tenant_patient_class_active` (partial), `idx_patient_allergies_tenant_patient`. cdss_check_results: `idx_cdss_check_results_tenant_patient_status`, `idx_cdss_check_results_tenant_type_status`. ai_features: `uq_ai_features_tenant_facility_function`, `idx_ai_features_tenant_facility_status_enabled`. ai_drafts: `idx_ai_drafts_tenant_facility_status`, `idx_ai_drafts_tenant_patient`. |
| **State machine** | Rule: draft → active (superseded). Check result: open → overridden. AI feature: registered → active (retired); enabled ⇄ disabled. Draft: draft → signed \| withdrawn. |
| **Audit** | `cdss_rule.stored/.activated`, `cdss_check.run/.degraded`, `cdss_check.overridden`, `cdss_pathway.evaluated`, `ai_feature.registered/.activated/.enabled/.disabled`, `ai.invoked/.invoke.degraded`, `ai.draft.created/.signed/.withdrawn` — facts and ids only; never check-result messages, draft text, prompts, or outputs (AI_RULES.md §11, OBSERVABILITY.md §17 — proved by CdssTest and AiGovernanceTest PHI-safety legs). |
| **Soft deletion** | None — rules are versioned/superseded, results are lifecycle-managed, drafts are signed or withdrawn, features are kill-switched/retired |
| **Later-phase plan** | The actual inference service (ARCHITECTURE.md §28.5) with a registered, evaluated, allowlisted model — the governance surface, gating, and audit are complete; the model itself is NOT PROVEN and no AI runs today. CDSS rule authoring UI; pathway library expansion |
