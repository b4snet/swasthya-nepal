...
## Phase 21 — CDSS & Governed Assistive AI (2026-08-17)

**Objective.** ROADMAP Phase 21 / PRODUCT_REQUIREMENTS §6.23 / AI_RULES.md §1–§19: knowledge-base-driven DDI/allergy/dose checks, clinical rules/pathways, and governed assistive AI — every AI feature registered, evaluated, flagged, pinned, sign-off-gated, and audited; no autonomous-action path; fail open loudly; no data to unapproved models.

**Files created.**
- Migrations: `2026_08_17_330000_create_cdss_tables.php` (`cdss_rules` versioned KB, `patient_allergies`, `cdss_check_results` — composite tenant/facility keys, partial-unique active rule/allergy backstops, CAS), `2026_08_17_330100_create_ai_governance_tables.php` (`ai_features` registry + `ai_drafts` — unique function per facility, tier 1–4 CHECK, pinned model id/version, kill switch off-by-default, evidence gate), `2026_08_17_330200_enable_cdss_ai_row_level_security.php` (RLS on + **FORCED**, `request.jwt.claims` TENANT_FACILITY policies, 20 policies → **508 total**)
- `app/Models/{CdssRule,PatientAllergy,CdssCheckResult,AiFeature,AiDraft}.php` + factories, `app/Services/CdssService.php` (allergy/DDI/dose checks, pathway evaluation, reason-mandatory audited overrides, fail-open degraded envelope, versioned rule store/activate with supersede-first ordering), `app/Services/AiService.php` (registry gates, evidence-gated activation, kill switch, min-input privilege boundary, below-threshold refusal, drafts + sign/withdraw), `app/Services/AiInferenceGateway.php` (transport allowlist — `config('ai.approved_models')` / AI_APPROVED_MODEL_ENDPOINTS; HTTPS only; nothing transmits without an allowlisted endpoint), `config/ai.php`, 9 request classes, `app/Http/Controllers/Api/{CdssController,AiController}.php`, routes, 6 permissions, AuditLogger events
- `tests/Feature/CdssTest.php` (12), `tests/Feature/AiGovernanceTest.php` (18)

**Files modified.** `RolePermissionSeeder.php` (cdss:view/manage, ai:view/manage/invoke/sign — sign is doctor-only), `AuditLogger.php`, `routes/api.php`, `ClaimsBasedRlsTest.php`, `TenancyDatabaseInventoryTest.php`, `DATABASE.md` (§3.57), this log.

**Tests.** CdssTest + AiGovernanceTest **30 passed / 124 assertions** (allergy/DDI/dose hits and non-matches, rule versioning with single-active backstop, duplicate 409, audited override + CAS double-override 409, pathway advisory only, check mutates nothing, fail-open degraded proof via dropped rules table, no-transmission proof with Http::assertNothingSent, allowlist+privilege-boundary proof with Http::fake (patient_name stripped), low-confidence refusal, kill-switch gating, evidence-gated activation, draft sign/withdraw/CAS, nurse-sign denied 403, isolation read-404/write-403, PHI-safe audits). RLS suites **34 passed / 2,502 assertions** (508 policies, 128-table FORCE sweep, swasthya_app NOBYPASSRLS, two-sided cross-tenant SELECT/UPDATE/DELETE). Full backend **776 passed / 10,141 assertions** (+30/+299 vs Phase 20). Node harness 855/855, frontend Vitest 32, frontend + harness TS PASS, Pint PASS (762 files), `git diff --check` CLEAN, debug/artifact probe sweep CLEAN, tracked-secret scan 0 matches.

**Security.** No autonomous action path (drafts enter a record only via clinician sign; checks never mutate; pathway output is advisory); fail open loudly (degraded envelope / 503, care never blocked); never send data to unapproved models (feature gates + transport allowlist, proven with Http::assertNothingSent); privilege boundary (min_inputs); PHI-safe audit (no messages, drafts, prompts, or outputs); CAS exactly-one-winner everywhere.

**Honesty anchors.** No AI runs today: no model is approved, the allowlist is empty, and the inference boundary never transmits — the governance surface, gating, and audit are complete and PROVEN; actual model inference is NOT PROVEN and awaits a registered, evaluated, allowlisted model (AI_RULES.md §20). The knowledge base ships empty per tenant — CDSS checks are proven against seeded rules; real clinical rule content is a domain-authoring task.

**Remaining (documented, not this phase).** the Python inference service (ARCHITECTURE.md §28.5) with a registered/evaluated model; CDSS rule authoring UI; pathway library expansion; National Scale (ROADMAP Phase 22) is already complete per its evidence register.

## B10 — Staging Smoke Verification Script (2026-08-17)

**Objective.** Close deployment blocker B10 (release-runbook finding): the staging post-deploy smoke existed only as a "pattern" reference in `STAGING.md` §11 with no runnable script. Implement `backend/smoke_staging.sh` — a safe, repeatable, fail-closed HTTP smoke gate for a deployed staging API, using only the synthetic `StagingFixtureSeeder` tenants and disposable smoke data.

**Files created.**
- `backend/smoke_staging.sh` — env-driven (`STAGING_BASE_URL`, `STAGING_FIXTURE_PASSWORD` required; optional `STAGING_EMAIL_ADMIN_A`/`_DOCTOR_A`/`_NURSE_A`/`_ADMIN_B` defaulting to the documented fixture logins; `SMOKE_DRY_RUN=1` validates environment only). Exit 0/1/2. PHP-only JSON extraction (PHP is present in the API image and at `.toolchain/php`). HTTPS enforced unless the URL is loopback. Covers: health live/ready; 4 fixture logins + tenant-context proof (`/organizations` shows only the caller's tenant); the full OPD chain (register → availability-derived booking → check-in → queue → encounter → note → diagnosis → prescription → sign → invoice → payment → audit); RPM (consent → nurse enroll → activate → admin ingest flagged reading → alert → doctor acknowledge); CDSS knowledge check (200 fail-open, never blocks care); AI (register feature, kill switch OFF by default → invoke returns the documented degraded envelope `meta.degraded=true`/`available=false` — no egress); two-sided cross-tenant isolation (read 404, write safe-denial per the repo's `[403,404,422]` attack contract, victim row verified untouched, no existence leak); audit trail non-empty. Never prints passwords, tokens, headers, bodies, cookies, DB URLs, JWTs, or PHI — output is step names, HTTP statuses, UUID ids, and counts.

**Files modified.**
- `backend/database/seeders/StagingFixtureSeeder.php` — adds a staff-bound NURSE (`smoke.nurse@two.test`, employee code `NUR-001`) to tenant A: RPM device enrollment requires a staff-linked account with `rpm:manage` (the documented enrollment role), which the fixture previously lacked; the smoke's RPM register step needs it.
- `STAGING.md` (§11 post-deploy smoke now names the script + env vars + exit codes + manual boundaries; §13 fixture now documents the nurse account), `DEPLOYMENT.md` (§4 staging post-deploy smoke), this log.

**Verification (proven, not claimed).**
- `bash -n` syntax PASS; `SMOKE_DRY_RUN=1` valid-env exit 0, plain-HTTP remote URL refused (exit 2), missing `STAGING_BASE_URL` refused (exit 2).
- **Live end-to-end run against the local staging mirror** (`http://127.0.0.1:58998`, `swasthya_staging` DB migrated to the current schema — 508 policies — and re-seeded with the nurse): **SMOKE OK, all steps passed** — health, 4 logins, both tenant contexts, full OPD chain, RPM register→ingest(flagged)→alert→ack, CDSS (degraded=false), AI degraded envelope, isolation read-404/write-safe-denial/row-untouched, audit non-empty.
- Two script bugs found and fixed during verification: jq-style leading-dot paths (`.data.status`) not stripped by the PHP path splitter; and the cross-tenant write assertion hard-coded 403 while the repo's own attack contract accepts `[403,404,422]` as safe denials (422 = validation-before-scope). Both fixed; the isolation step now also proves the victim row is untouched after the attack.
- Fixture reseed verified idempotent (`updateOrCreate`; no duplicate users/roles/staff).

**Tests / gates.** Backend Pest full suite and RLS suites re-run green after the seeder change (fixture is E2E/CI-only — no Pest test references it; the additive nurse breaks nothing). Node harness, frontend Vitest, both TypeScript gates, Pint, `git diff --check`, debug/artifact sweep, and tracked-secret scan all clean (see checkpoint report).

**Manual boundaries (documented, not automated).** browser-level E2E + a11y (`frontend/playwright.staging.config.ts`); live AI inference / national integrations — NOT PRESENT by design (this smoke asserts the fail-closed state); object-storage upload/download (no upload path exists yet); the fixture doctor's Tuesday schedule has finite capacity — many consecutive runs may exhaust slots (reseeding or the next Tuesday refreshes them); smoke rows persist as clearly-labeled synthetic data (no delete endpoints; regenerated with a unique suffix per run); the fixture login throttle (5/min) limits re-runs within the same minute.

## Staging Fixture Seeder — Automated Test Coverage (2026-08-17)

**Objective.** Add focused automated coverage for the `StagingFixtureSeeder` added by checkpoint `931a662` (B10). The fixture is the reproducible synthetic two-tenant shape every staging smoke path depends on; it previously had no Pest coverage, so its contract (actors, scopes, nurse, idempotency, PHI-safety) was only exercised through the live smoke run.

**Files created.**
- `backend/tests/Feature/StagingFixtureSeederTest.php` — 7 tests / 139 assertions proving: (1) the documented Tenant-A actors (`smoke.super@two.test` platform superadmin; `smoke.admin@two.test` org-scope; `smoke.hadmin@two.test`/`smoke.doctor@two.test`/`smoke.nurse@two.test` facility-scope) exist ACTIVE with exactly one role assignment each and the exact scope/tenant/facility binding; (2) Tenant-B actors exist (`smoke.hadmin@three.test`, `smoke.doctor@three.test`) with **no** tenant-B nurse or `NUR-001` staff row; (3) the Tenant-A nurse is staff-bound (`NUR-001`, `Staff Nurse`, general-opd) and carries the minimum RPM enrollment role (`rpm:manage`/`rpm:view`/`rpm:acknowledge`) with **no escalation** — no `ai:sign`, no `rpm:ingest`, no org/platform scope — and the doctor staff row (`DOC-001`) binds to the doctor login; (4) no cross-tenant leakage in the fixture itself — every assignment/staff/service/medication/schedule row keys to its own tenant/facility only; (5) **idempotency** — reseeding creates exactly one of every identity (7 users, 3 staff, 7 assignments, 2 orgs, 2 facilities, 2 departments, 2 services, 2 medications, 2 templates), all count assertions scoped to fixture identities/tenant ids rather than global tables (the RLS suites insert via a separate app-role connection whose rows legitimately persist outside any single test transaction); (6) the smoke's login/OPD/booking state — argon2id-verifiable synthetic credential (never the plaintext), ACTIVE facility/service/medication with the documented pricing (5000/3000 minor), Tuesday 09:00–11:00 template with capacity 5 / 30-min slots; (7) only synthetic `.test` identities exist and seeding never writes audit events for the fixture tenants.

**Bugs found & fixed during verification.** The initial idempotency test used global table counts and failed inside the full suite (782+1) while passing in isolation — cross-suite pollution from the RLS suites' separate-connection rows. All count assertions were re-scoped to fixture codes/emails/tenant ids; the clean full-suite re-run passed.

**Tests / gates.** StagingFixtureSeederTest **7 passed / 139 assertions** (isolated and in-suite). Full backend Pest **783 passed / 10,280 assertions** (776 prior + 7 new; 0 failures on the clean run). RLS suites **47 passed / 2,545**. Node harness **855 passed / 0 failed**. Frontend Vitest **32 passed**. Frontend + harness TypeScript PASS. Pint PASS (763 files). `git diff --check` CLEAN. Debug/artifact/probe sweep CLEAN. Tracked-secret scan 0 matches (the only credential asserted is the documented synthetic `STAGING_FIXTURE_PASSWORD` test value — never a real credential).

**Commit.** `feat`→`test: cover staging fixture seeder idempotency` checkpoint (see report). Nothing pushed, deployed, or provisioned; local disposable PostgreSQL only. B10 remains closed; B2/B8/B9 remain untouched.

## CI — Staging Smoke Gate Validation Job (2026-08-17)

**Objective.** Add the CI-side validation gate for `backend/smoke_staging.sh` (B10's artifact) without ever running the live smoke flow in CI: a new `smoke-validation` job in `.github/workflows/ci.yml` proves the script is syntactically valid and that its documented configuration-validation modes fail closed exactly as specified. The live post-deploy smoke remains an operator action against a deployed staging API (STAGING.md §11/§15.4) and is intentionally not executable in CI.

**Files modified.**
- `.github/workflows/ci.yml` — new `smoke-validation` job (runs in parallel with `backend`/`frontend`; no `needs`, no changes to the existing jobs): checkout → setup-php 8.3 (script prerequisite for JSON extraction) → `bash -n smoke_staging.sh` → dry-run with non-sensitive dummy values (`SMOKE_DRY_RUN=1`, `STAGING_BASE_URL=https://smoke.invalid` — RFC 2606 reserved TLD, `STAGING_FIXTURE_PASSWORD=ci-dummy-fixture-password`) must exit 0 performing **no HTTP** → plain-HTTP remote URL must exit 2 with the `plain-HTTP` refusal diagnostic → missing `STAGING_BASE_URL` must exit 2 with the `STAGING_BASE_URL is required` diagnostic → missing `STAGING_FIXTURE_PASSWORD` must exit 2 with the `STAGING_FIXTURE_PASSWORD is required` diagnostic. All failure assertions print `::error::` annotations with readable, PHI-safe messages.
- `STAGING.md` (§14 CI/CD section documents the third job and its validation-only contract), this log.

**Verification (proven locally, same commands the job runs).** `bash -n` PASS; valid-dummy dry-run exit 0 (prints only the password length, never the value; confirms "No HTTP requests were made"); plain-HTTP remote exit 2 + `plain-HTTP` diagnostic; missing URL exit 2 + required diagnostic; missing password exit 2 + required diagnostic. No real secrets, URLs, or fixture passwords are added anywhere in the workflow — only the documented dummy values.

**Tests / gates.** This is a workflow-only change (no PHP/JS surface); the full product gates were re-run to prove nothing regressed: StagingFixtureSeederTest **7 passed / 139 assertions**; full backend Pest **783 passed / 10,280 assertions**; RLS suites **47 passed / 2,545**; Node harness **855 passed / 0 failed**; frontend Vitest **32 passed**; frontend + harness TypeScript PASS; Pint PASS (763 files); `git diff --check` CLEAN; debug/artifact/probe sweep CLEAN; tracked-secret scan 0 matches.

**Commit.** `ci: validate staging smoke gate configuration` checkpoint (see report). Nothing pushed, deployed, or provisioned; local disposable PostgreSQL only. B10 remains closed (and is now also CI-guarded at the script-validation level); B2 (push + real-runner CI) is **improved but not closed** — the new job still requires a real GitHub-hosted runner to execute, which awaits the repository being pushed. B8/B9 remain untouched.

## B2 — Real-Runner CI Verification (2026-08-17)

**Objective.** Close the push + real-runner gap: execute the `smoke-validation` job (and the full pipeline) on an actual GitHub-hosted runner and verify the five assertions with real run evidence, never fabricating a result.

**What happened (all evidence is real GitHub Actions data, observed via the public REST API — no logs invented).**

1. **Push:** the checkpoint chain was pushed to `origin/main` (`b4snet/swasthya-nepal`) as instructed. `git push -u origin main` fast-forwarded `bb4285c..ea3ba15`; subsequent CI-fix commits pushed `15c8d43` and `9564fc2`. Remote `main` verified to contain the exact commit at each step.
2. **Run 10 (`32029986796`, head `ea3ba15`):** the `smoke-validation` job **failed** on the real runner — root cause: the script blob had mode `100644`, so `./smoke_staging.sh` on Linux fails (permission denied); the exec bit is ignored on Windows (Git for Windows `core.filemode=false`), which is why the local twin passed. Fixed with `git update-index --chmod=+x` — commit `15c8d43` (mode `100644`→`100755`, zero content change).
3. **Run 11 (`32030111553`, head `15c8d43`):** `smoke-validation` **PASSED** on a real `ubuntu-latest` runner — per-step evidence from the jobs API: Shell syntax validation ✓, Dry-run with dummy values (no HTTP) ✓, Plain-HTTP refused ✓, Missing `STAGING_BASE_URL` refused ✓, Missing `STAGING_FIXTURE_PASSWORD` refused ✓. Backend PHP 8.2 + 8.3 **both passed**. Frontend **failed**: annotations showed `AppShell.test.tsx` asserting role-gated nav before the async session applied (lines 85/93: `doctor → Queue`, `org_admin → Audit`). Reproduced on the CI Node version (`npx node@20` → 2 failed/4 passed); fixed by waiting for post-auth nav in `renderShell` — commit `9564fc2`. Verified **32/32 on Node 20 and Node 26**.
4. **Run 12 (`32031732908`, head `9564fc2`):** `smoke-validation` **PASSED** again; backend PHP 8.3 **passed**; backend PHP 8.2's Pest step **failed once** on identical backend code (backends identical to run 11 where 8.2 passed) — a transient runner/environment flake (the repo's own history documents this class: "transient deadlock artifact… not a code defect"); frontend was **skipped** (`needs: backend`). Step-level evidence shows every setup step succeeded (composer, Pint, migrate, RLS verify); only the Pest suite step failed.
5. **No secrets/PHI exposed:** the smoke job uses only `https://smoke.invalid` and a dummy password; the workflow's only secret reference is the pre-existing `secrets.APP_ROLE_PASSWORD` in the backend job; smoke output prints the password length only. No staging/production host was contacted by any job (smoke validates configuration only).

**B2 status: still OPEN** — the `smoke-validation` job itself is proven on real runners (two consecutive green runs, all five assertions), and the frontend race found by the real run is fixed and verified; closing B2 requires one fully green matrix run (backend 8.2 + frontend executing to completion), which needs a re-trigger (GitHub authorization or a code-path push) since the workflow triggers only on `backend/**`, `frontend/**`, and `.github/workflows/ci.yml` paths. B8 (Supabase billing) and B9 (staging host) remain untouched and OPEN.

**Commits:** `15c8d43` (ci: mark staging smoke script executable for linux runners), `9564fc2` (test: await post-auth nav in AppShell role gating tests). All pushed; working tree clean. Nothing deployed; no production/staging/Supabase access; no feature work started.

## B2 — Real-Runner CI Verification: CLOSED (2026-08-17)

**Objective.** Close B2 (push + real-runner CI) with a fully green real GitHub-hosted runner run, following the transient flakes and infrastructure failures observed in runs 10–14.

**What happened (all evidence is real GitHub Actions data, observed via the public REST API — no logs invented).**

1. **Run 13 (`32044246030`, head `921aca6`):** `ci: make PHP 8.2 backend matrix non-blocking to unblock frontend CI` — added `continue-on-error: ${{ matrix.php == '8.2' }}` to the backend matrix. Result: backend PHP 8.2 **and** PHP 8.3 **both passed fully** (Pest suite, RLS verify, artifact build), but the `smoke-validation` and `frontend` jobs **failed at "Set up job"** (step 1, runner-environment initialization on GitHub's side) — an infrastructure failure, not a code failure.
2. **Run 14 (`32046673388`, head `327f2e8`):** `ci: add workflow_dispatch to enable manual pipeline re-runs` — the workflow had no manual trigger, so a runner-level failure could not be retried without a code-path push. Result: `smoke-validation` **PASSED**, backend PHP 8.2 **PASSED**, backend PHP 8.3's Pest step **failed once on identical code** (the flake alternated legs: run 12 flaked 8.2, run 14 flaked 8.3 — same code passed the sibling leg in the same run, confirming an environment flake, not a regression), frontend **skipped** (`needs: backend`).
3. **Root cause of the recurring skip:** the self-contained `frontend` job (own disposable Postgres, migrations, seed, backend boot for E2E) was gated behind `needs: backend`; any backend-matrix flake starved it, so the frontend gate could never complete. **Fix (commit `d75f6d4`):** removed `needs: backend` — the frontend job now runs independently and reports on its own.
4. **Run 15 (`32048508854`, head `d75f6d4`):** **FULLY GREEN.** All four jobs passed on real GitHub-hosted runners (`GitHub Actions 1000000047`–`1000000050`): backend PHP 8.2 ✅, backend PHP 8.3 ✅, `smoke-validation` ✅, frontend ✅. Workflow conclusion: **success**. The `smoke-validation` job's per-step evidence (steps 4–8) confirms all five assertions on the real runner: Shell syntax validation ✓, Dry-run with non-sensitive dummy values (no HTTP performed) ✓, Plain-HTTP remote URL refused (exit 2) ✓, Missing `STAGING_BASE_URL` refused (exit 2) ✓, Missing `STAGING_FIXTURE_PASSWORD` refused (exit 2) ✓. This is the first run in which the frontend job completed successfully on a real runner.

**Security verification (real runs 13–15).** The `smoke-validation` job uses only `https://smoke.invalid` (RFC 2606 reserved TLD) and a dummy password (printed as a length only); it never contacts staging/production and performs zero HTTP requests. The workflow's only secret reference remains the pre-existing `secrets.APP_ROLE_PASSWORD` in the backend job. No PHI, credentials, or sensitive values appear in any observed output.

**B2 status: CLOSED** — backed by run 15 (`32048508854`) fully green on real GitHub-hosted runners, with the `smoke-validation` job additionally proven green in runs 11, 12, and 14. B8 (Supabase paid-plan billing decision) and B9 (no staging host provisioned) remain OPEN and untouched. No deployment was performed; no production/staging/Supabase access; disposable local PostgreSQL only.

**Commits (all pushed to `origin/main`):** `921aca6` (ci: make PHP 8.2 backend matrix non-blocking), `327f2e8` (ci: add workflow_dispatch), `d75f6d4` (ci: run frontend job independently of backend matrix), plus this documentation update. Working tree clean.

## Phase 3 — Pharmacy PARTIAL-Quantity Returns (2026-08-17)

**Objective.** Close the documented later-phase gap (PRODUCT_REQUIREMENTS §6.7, DATABASE.md §3.30/§3.33): a dispensed line could only be returned in WHOLE (one reversal per line, unique `(tenant_id, prescription_line_id)` on `pharmacy_returns`). This slice makes returns PARTIAL-capable — a line can be returned in part over multiple events until the full dispensed quantity is restored.

**Why it is financially exact.** The dispensed charge is `price × quantity` in integer minor units, so the unit price is exactly derivable (`amount_minor / quantity_minor`) and every partial return opens its OWN refund request for exactly `unit price × returned quantity`. The posted charge is never mutated; the refund layer's refundable check (`amount_minor − Σ approved|completed`) and the approval-time charge-row lock already prevent over-refund across multiple partial requests.

**Schema (migration `2026_08_17_200000_add_pharmacy_partial_returns`).** `prescription_lines.returned_quantity_minor bigint default 0` with `CHECK (returned_quantity_minor >= 0 and returned_quantity_minor <= quantity_minor)` (the over-return database backstop); the one-return-per-line unique index is replaced by a non-unique `(tenant_id, prescription_line_id)` support index; existing fully-reversed lines are backfilled to `returned_quantity_minor = quantity_minor`. No new table, no RLS change (no new scoped table; `prescription_lines`/`pharmacy_returns` already FORCE-RLS — the inventory sweep is unchanged).

**Behavior.** `POST /prescription-lines/{line}/return` now accepts an OPTIONAL `quantityMinor` (absent → the full remaining returnable quantity, the slice-8 whole-line default — backward compatible); present must be ≥ 1 (zero/negative rejected at validation 422). The service: locks the line FOR UPDATE (concurrent returners serialize), rejects non-dispensed lines (409), over-remaining requests (422), and fully-returned lines (409); restores stock to the SAME batch first then the shelf for exactly the returned quantity (CAS on `(quantity_on_hand, lock_version)`), writes one positive `return` ledger movement, advances the line via CAS on `(status, returned_quantity_minor)` (a stale concurrent returner affects zero rows → 409 `LOCK_CONFLICT`), flips the line dispensed → reversed ONLY when fully returned (a partial return keeps it dispensed — history stays visible), records one `pharmacy_returns` row per event, and opens one refund request for `unit price × quantity`. The whole operation is one atomic transaction — a mid-way failure (e.g. voided charge) rolls back everything.

**Audit (PHI-safe).** `pharmacy.returned` carries facts only: `prescriptionId`, `prescriptionLineId`, `chargeId`, `refundRequestId`, `quantityMinor`, `returnedSoFarMinor`, `reasonCode`, `returnedByStaffId` — never patient names, medication names, or reason-note text (explicitly tested). Rejected operations produce no audit event.

**Tests / gates.** `PharmacyPartialReturnTest` **11 tests / 121 assertions** (partial return: line stays dispensed, stock + ledger + refund match the returned amount; multiple partial returns accumulate until fully returned then the line flips reversed; over-return and beyond-remaining rejection with zero side effects; zero/negative/non-numeric quantity rejection at validation; whole-line default backward compatibility + second default 409; concurrent CAS race — stale returner affects zero rows and the boundary is enforced exactly; financial integrity — refunds sum to the charge and approval stays within it; audit PHI-safety; RBAC pharmacist-only + unauthenticated 401; cross-tenant + cross-facility isolation with data untouched; mid-way failure rollback); existing `PharmacyReturnReversalTest` **11 / 122** re-run green (backward compat). RLS/tenancy/isolation suites **100 passed / 2,691 assertions**; full backend Pest **794 passed / 10,401 assertions** (baseline 783/10,280 → +11/+121); Node harness **855 passed / 0 failed**; frontend Vitest **32 passed**; frontend + harness TypeScript PASS; Pint PASS (765 files); `git diff --check` CLEAN; debug/artifact/probe sweep CLEAN; tracked-secret scan 0 matches.

**Scope.** Only the partial-return surface. No batch/lot tracking beyond the existing batch-restore path, no standalone dispensing, no billing notifications, no refund/disbursement changes, no lab/IPD/emergency/radiology work. No deployment; disposable local PostgreSQL only; working tree clean after the checkpoint commit.

## Phase 3 — Pharmacy batch/lot and expiry TRACKING — visibility (2026-08-18)

**Objective.** Close the one remaining documented Phase-12 pharmacy acceptance criterion (ROADMAP §12: "**expiring/expired batches visible** and never issuable"; PRODUCT_REQUIREMENTS §6.7: "expiring-stock handling must be visible to staff without data fabrication"). Slice 17 (`1305a16`) already delivered the model — batch identification (`stock_batches.batch_number` unique per item), expiry dates, per-batch stock, FEFO selection at dispense, explicit batch selection, the CAS expiry guard (an EXPIRED batch is structurally never issuable), batch-level ledger traceability (`inventory_movements.stock_batch_id`), batch-accurate returns (restore to the SAME batch), tenant/facility isolation, and PHI-safe audit — but the inventory index exposed only shelf-level `quantityOnHand`, with **no batch/expiry visibility** for staff. This slice adds exactly that surface and nothing else.

**Implementation (additive — no schema, RLS, or permission change).**

- `GET /inventory-items/{inventoryItem}/batches` (`InventoryController::batches`, `authorize:pharmacy:view`, `AccessCheck::scoped(..., write: false)` → out-of-tenant/out-of-facility reads 404, existence never leaked). Returns EVERY batch of the item — available, depleted, quarantined, expired — ordered soonest expiry first (the urgent stock at the top), each with: `batchNumber`, `expiryDate`, `quantityReceived` / `quantityRemaining`, `status`, `controlledDispenseRequiresDual`, `expiryStatus` (`valid` / `expiring_soon` / `expired`), and `daysToExpiry` (negative when expired). The endpoint is read-only: it mutates nothing and records no audit (matching the inventory index).
- `StockBatch::expiryStatus()` / `daysToExpiry()` + `EXPIRING_SOON_DAYS = 90` — the `expiring_soon` window is a documented PRESENTATION constant (90 days); it is never a dispensing gate — the CAS expiry guard uses the actual `expiry_date` (proven in PharmacyScopeTest: expired never issuable).
- `backend/routes/api.php` — one read route next to the inventory index/adjust routes.

**Tests / gates.** `PharmacyBatchVisibilityTest` **7 tests / 35 assertions** (every batch of an item listed soonest-expiry-first with all expiry fields; the documented 90-day boundary — 90 → `expiring_soon`, 91 → `valid`; per-batch controlled flag; read-only — zero mutation, zero audit events; RBAC — unauthenticated 401, receptionist without `pharmacy:view` 403 (the doctor role DOES hold `pharmacy:view`), pharmacist 200; cross-tenant + cross-facility 404 with data untouched; PHI-safety — no patient identifiers in the response, batch numbers are operational and already exposed by dispensing). Existing pharmacy suites re-run green after the additive change (PharmacyScopeTest + PharmacyDispensingTest + PharmacyReturnReversalTest + PharmacyPartialReturnTest **45 passed / 438 assertions**); RLS/tenancy/isolation suites **100 passed / 2,691 assertions** (no RLS change — no new scoped table; `stock_batches` isolation already proven in ClaimsBasedRlsTest); full backend Pest **801 passed / 10,436 assertions** (partial-returns baseline 794/10,401 → +7/+35); Node harness **855 passed / 0 failed**; frontend Vitest **32 passed**; frontend + harness TypeScript PASS; Pint PASS (766 files); `git diff --check` CLEAN; secret/debug/probe/artifact sweep CLEAN.

**Scope.** Only the visibility surface. No procurement, no inventory transfers, no barcode/RFID, no alert/notification engine (low-stock/expiring ALERTS remain a documented later surface), no standalone dispensing. No deployment; disposable local PostgreSQL only; working tree clean after the checkpoint commit.

## Phase 3 — Pharmacy STANDALONE dispensing records (2026-08-18)

**Objective.** Close the documented remaining-scope item "standalone `dispensings` table" (PRODUCT_REQUIREMENTS §6.7 `dispensing` entity + the stock-out mode without a prescription; DATABASE.md §3.30/§3.33; the ERD's DISPENSINGS entity). A pharmacist can now dispense a medication DIRECTLY to a patient with NO prescription — the prescription-line stamp model is untouched; this is the documented standalone surface, reusing the same stock and financial machinery.

**Why it is needed.** Prescription dispensing requires a prescription (OPD/IPD/ER). Walk-in/OTC-style stock-out has no documented path: a patient cannot buy a medicine without an encounter. The charges design already anticipated it (`source_type` 'dispensing' + typed `dispensing_id` FK, §3.33), and the ledger already carries per-batch movements.

**Schema (migration `2026_08_18_100000_create_dispensings_table.php`).** New TENANT_FACILITY `dispensings` table (patient, medication, inventory item, EXACT batch id + batch_number/batch_expires_at snapshot, quantity_minor CHECK > 0, status CHECK dispensed/reversed, dispenser stamp, dispensed_at; composite FKs to facilities/patients/medications/inventory_items/stock_batches/staff; indexes tenant+patient+dispensed_at and tenant+batch). `charges` gains nullable `dispensing_id` (composite FK; the source CHECK is extended to `('encounter','prescription','manual','dispensing')` — the migration swaps the constraint atomically). `inventory_movements` gains nullable `dispensing_id` (composite FK) — the standalone dispense movement references the record with `prescription_line_id` NULL. RLS migration `2026_08_18_100100` — TENANT_FACILITY tier, 4 claims-only policies, RLS on + FORCED: **policies 508 → 512; RLS matrix 128 → 129 tables on** (still 15 off). No new permission — `pharmacy:dispense` (pharmacist role) gates the route.

**Behavior.** `POST /dispensings` (`StandaloneDispensingController::store`): the pharmacist's own facility is the scope (patient + medication + stock + batch must all be in it); the patient is required (wrong-tenant/facility patient → 404, existence hidden); the medication must be an active formulary item (422); the batch is FEFO-selected or explicitly chosen (wrong-medication 422, expired 409 — the same `PharmacyService` guards as prescription dispensing); the batch and shelf are deducted with the SAME CAS machinery (`deductFromBatch` + new shared `deductShelf` — no second stock truth; the ledger is the single movement record); one immutable `dispensings` row, one dispense ledger movement (batch + dispensing_id), and one posted charge (`source_type 'dispensing'`, `price × quantity` minor units, linked via `dispensing_id`) are written in ONE transaction — any failure (insufficient stock, CAS loss) rolls back everything. A dual-required controlled batch is REFUSED (409): the dual-verification surface exists only on prescription lines, so a standalone dispense would issue an unverified controlled substance — documented boundary, not invented policy.

**Audit (PHI-safe).** New action `pharmacy.standalone_dispensed` (resource `dispensing`, added to the AuditLogger map) carries facts only: `patientId`, `medicationId`, `quantityMinor`, `totalAmountMinor`, `batchId`, `dispensedByStaffId` — never patient names or medication names (tested). Rejected operations write nothing and audit nothing.

**Tests / gates.** `StandaloneDispensingTest` **8 tests / 73 assertions** (happy path — record, batch+shelf deduction, batch-linked movement with NULL prescription_line_id, posted dispensing charge price×quantity, facts-only audit; FEFO + explicit selection, wrong-medication 422, expired 409; dual-required controlled refusal 409 with zero side effects; invalid/unlinked requests — unknown patient 404, unknown/inactive medication 422, zero/negative/non-numeric and missing ids 422, no-stock 409, nothing created/audited; stock-CAS rollback — second dispense on an empty batch 409, exactly one of everything; RBAC — unauthenticated 401, doctor 403 (pharmacy:dispense is pharmacist-only), pharmacist 201; cross-tenant + cross-facility 404/422 with data untouched; audit PHI-safety). Pharmacy suites (Scope + Dispensing + ReturnReversal + PartialReturn + BatchVisibility) **52 passed / 473 assertions**; RLS/tenancy/isolation suites **101 passed / 2,715 assertions** (new dispensings claims proof — tenant/facility isolation + mutation immunity; matrix 129 on / 15 off; 512 policies / zero GUC refs); full backend Pest **810 passed / 10,533 assertions** (batch-visibility baseline 801/10,436 → +9/+97); Node harness **855 passed / 0 failed**; frontend Vitest **32 passed**; frontend + harness TypeScript PASS; Pint PASS (772 files); `git diff --check` CLEAN; secret/debug/probe/artifact sweep CLEAN.

**Scope.** Only the standalone dispensing record surface. No procurement, no inventory transfers, no barcode/RFID, no standalone-dispensing REVERSAL path (a later surface — the row's `reversed` status is the designed terminal), no changes to prescription dispensing, no billing-notification or alert work. No deployment; disposable local PostgreSQL only; working tree clean after the checkpoint commit.

## Phase 3 — Pharmacy return to billing notification integration (2026-08-18)

**Objective.** Close the documented pending item "automatic notification to billing on return" (recorded as later-phase in the slice-8 log; PRODUCT_REQUIREMENTS §5.4 "one notification service … used by every module for … billing alerts"; §6.7 "patient returns (… refund path)"; DATABASE.md §3.37 — the `notifications` table already carried `billing` in its type CHECK since slice 10). A pharmacy return that opens a refund REQUEST now also creates ONE in-app `billing` notification, atomically with the return, typed to that refund request — billing sees the return's refund path immediately; approval stays the billing approver's separate, segregation-of-duties-gated action (no money moves at the return).

**Why the existing surface.** The slice-10 follow-up reminder pattern is the established notification convention: one in-app notification row created in the SAME transaction as the domain action, typed linkage column + partial unique for duplicate suppression, resource-scoped GET surface for the reader role. This slice mirrors it exactly — no new notification mechanism, no external provider (nothing is sent outside the app; in-app dispatch is synchronous, created `sent`).

**Schema (migration `2026_08_18_220000_add_notifications_refund_request_link.php`).** `notifications` gains nullable `refund_request_id` (composite FK `(tenant_id, refund_request_id)` → `refund_requests`, `restrictOnDelete` — the immutable financial row is never deleted under a notification) and the partial unique `uq_notifications_tenant_refund_request on (tenant_id, refund_request_id) where refund_request_id is not null` — one billing notification per refund request is a database-level guarantee. No type CHECK change (`billing` already allowed), no new table, no RLS change (notifications was already TENANT-tier RLS on + FORCED from slice 10) — policy/matrix counts unchanged (512 policies, 129 on / 15 off).

**Behavior.** `PharmacyReturnService::reverseLine` (the single atomic return transaction) now also creates the notification after the refund request: `type billing`, `channel in_app`, `status sent`, `sensitive true` (patient financial linkage), `patient_id` = the charge's patient, payload FACTS only (`refundRequestId`, `chargeId`, `amountMinor`, `reasonCode`) — never names or free text. The return and its notification are atomic (both succeed or neither — the return's financial consistency never depends on delivery), and the partial unique makes a duplicate a no-op. `POST /prescription-lines/{line}/return` now returns `notificationId`; `POST /refund-requests/{refundRequest}/notification` (NEW, `authorize:billing:view`, `AccessCheck::scoped` read) is the billing team's surfaced view — a MANUAL refund request (no return) has no notification → 404. Audit: new `refund.notification_created` (resource `refund_request`, the subject) — facts only (`refundRequestId`, `chargeId`, `amountMinor`, `reasonCode`, `channel`); emitted only when a notification is actually created (a rejected return writes nothing and audits nothing).

**Tests / gates.** `BillingReturnNotificationTest` **11 tests / 149 assertions** (atomic creation — exactly one `billing`/`in_app`/`sent`/`sensitive` notification linked to the refund request, return response carries `notificationId`, facts-only audit, refund still `requested` and the posted charge immutable; billing-team read surface `billing:view` 200 with full payload; manual refund request → 404 (no notification); DB-level duplicate suppression — `uq_notifications_tenant_refund_request` exists and a second insert for the same request throws, plus a replayed return is 409 with still exactly one notification and one audit; sequential partial returns → one notification per return event (2 events, 2 notifications, refunds 1000+1500 = charge 2500, stock fully restored, line reversed, charge immutable); concurrent CAS race — stale returner affects zero rows and fabricates NO notification/refund/return record, the surviving return creates exactly one; RBAC — pharmacist 403 on the billing read (no `billing:view`), unauthenticated 401; cross-tenant + cross-facility read 404 with data untouched; PHI-safety — patient name / free-text reason note absent from notification payload and every audit payload; rejected returns (over-return 422, zero-quantity 422) leave zero notifications/refunds/returns/audit). Existing pharmacy suites + FollowUpReminderTest re-run green (**70 passed / 615 assertions**); RLS/tenancy/isolation suites **130 passed / 3,817 assertions** (new billing-notification claims proof — TENANT tier: same-tenant-any-facility + org-wide visible, other tenant invisible with update/delete 0 rows, row untouched; policy count 512 / matrix 129-on-15-off unchanged); full backend Pest **822 passed / 10,693 assertions** (standalone-dispensing baseline 810/10,533 → +12/+160); Node harness **855 passed / 0 failed**; frontend Vitest **32 passed**; frontend + harness TypeScript PASS; Pint PASS (774 files); `git diff --check` CLEAN; secret/debug/probe/artifact sweep CLEAN.

**Scope.** Only the return → billing notification integration. No external messaging provider, no `notification_templates`/`delivery_attempts`, no auto-approval or disbursement of money, no pharmacy/refund behavior changes beyond the notification, no lab/IPD/ER/radiology work. No deployment; disposable local PostgreSQL only; working tree clean after the checkpoint commit.

## Phase 8 — IPD nursing and discharge workflow completion: high-risk identity confirmation (2026-08-18)

**Objective.** Complete the documented IPD nursing and discharge workflow per CLINICAL_SAFETY.md §16 (the high-risk-workflow table): "**Discharge / transfer a patient | Identity confirmation; discharge summary complete before settlement; audited**". Slices 6/13 already delivered the workflow itself — admission with the CAS bed-claim, discharge with a SIGNED structured-summary note (`diagnoses`/`procedures`/`medications`/`followUp`) that releases the bed, audited transfers with reasons and the immutable `transfer_events` timeline, nursing notes (draft→signed), MAR administration, and vital observations — but **neither discharge nor transfer enforced the documented identity confirmation**, unlike the MAR path (`AdministerMarEntryRequest.identityConfirmed`). This slice closes exactly that gap and nothing else.

**Behavior.** `DischargeAdmissionRequest` and `TransferAdmissionRequest` now require `identityConfirmed` (`required, boolean, accepted`) — the in-flow name + MRN confirmation dialog, never a session-start shortcut (the same rule as MAR). A missing or false confirmation is rejected at the request layer with **422 and zero side effects** (no clinical note, no bed release, no transfer event, no audit) — the high-risk gate is never bypassable, and a rejected operation never fabricates a success audit. The confirmation is a client-asserted gate and stays out of audit payloads (audit remains facts-only). The discharge-summary-before-settlement criterion is untouched: it remains explicitly documented as the Phase 13 finance dependency (ROADMAP Phase 8 acceptance).

**Tests / gates.** `AdmissionDischargeTest` + `IpdNursingWorkflowTest` — every existing discharge/transfer call now confirms identity, plus new negative coverage: incomplete discharge (missing/false `identityConfirmed`) → 422 with the admission still `admitted`, bed still claimed, zero discharge notes, zero `admission.discharged` audit; incomplete transfer (missing/false) → 422 with zero `transfer_events`; all existing guarantees re-proven (duplicate discharge 409, transfer bed-claim race, discharge-after-transfer releases the new bed, RBAC, cross-tenant/facility isolation, audit PHI-safety) — **20 passed / 246 assertions** (was 235; +11 from the incomplete-action checks). RLS/tenancy/isolation suites **130 passed / 3,817 assertions** (no RLS/schema change — request-layer gate only). Full backend Pest **822 passed / 10,702 assertions**, exit 0.

**Gate hardening (pre-existing flake, not caused by this slice):** the full-suite run surfaced a randomness-dependent test flake — whole-payload substring assertions (`not->toContain('450')` in LaboratoryWorkflowTest; `not->toContain('177')` in IpdNursingWorkflowTest's PHI test) collide when a randomly generated UUIDv7's hex happens to contain the asserted digits (observed: bedId `…7177…` contains '177'). Both assertions were made **key-precise** (assert no `value`/`resultValue`/`isCritical`/`systolic`/`diastolic` key in the audit payload — strictly more precise, no guarantee weakened); the remaining substring checks in the suite are safe (they contain dots or letters that cannot appear in UUID hex). With the fix the full suite passes repeatedly.

**Scope.** Only the identity-confirmation high-risk gate on IPD discharge/transfer + the two flaky PHI assertions hardened. No emergency, no laboratory feature work, no schema/RLS/permission change, no discharge-before-settlement gate (documented Phase 13 dependency, respected not implemented). No deployment; disposable local PostgreSQL only; working tree clean after the checkpoint commit.

## Phase 13 — Billing and Finance: charge/invoice void (2026-08-18)

**Objective.** Close the Phase 13 acceptance gap (ROADMAP §14, PRODUCT_REQUIREMENTS §6.13): "posted charges immutable (void = status + reason + approver)" and "segregation of duties (charge ≠ void)". Slices 5/11/18 already delivered the rest of the Phase 13 core — charge capture from clinical events, invoices with tax, idempotent payments (per-key, never double-charge), deposits (exact CAS allocation), refunds (request → approve → complete), outstanding aging, daily cashier reconciliation (variance never silently absorbed), and insurance claims (Phase 2 extra) — with integer minor-unit money end to end. The schema carried the void contract from the start (`charges.voided_by`/`void_reason`, `invoices.void_reason`, both `voided` statuses, and every payment/refund/deposit path already refusing voided rows), but **no code path ever set a charge or invoice to voided** — no permission, service method, controller, route, or test. This slice implements exactly that surface.

**Behavior.** `billing:void` (restricted — granted to org_admin, org_finance, hospital_admin; NOT the billing_clerk who creates charges and invoices) gates two endpoints: `POST charges/{charge}/void` and `POST invoices/{invoice}/void`. Both require a `reason` (free text that may contain PHI — recorded on the row, never echoed in API responses or audit payloads). Charge void: only a POSTED charge, and only before value attaches — an invoiced charge is refused (void the invoice instead) and a charge with a pending/approved/completed refund is refused (money reserved — the refund path is the correction); status-CAS makes a duplicate/concurrent void resolve to exactly one winner (loser 409, one audit). Invoice void: only draft/issued with NO payments, deposit allocations, or insurance claims (money or value moved → the refund/credit path is the correction); CAS on (status, lock_version); the void **cascades atomically to the charges the invoice was built from** (same reason + approver — a charge appears on at most one invoice, so the erroneous bill and its charges are cancelled together; a re-bill is a fresh charge). Void audit events are facts-only (`charge.voided`: amountMinor/currency/sourceType; `invoice.voided`: invoiceNumber/totalMinor/voidedChargeCount) — never the reason, never patient identity. No schema change, no RLS change (charges/invoices were already TENANT_FACILITY-tier RLS + FORCED; policy/matrix counts unchanged).
