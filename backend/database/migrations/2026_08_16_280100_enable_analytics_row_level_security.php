...
/**
 * Phase 3 slice 21 — RLS for the Analytics and Reporting surface
 * (kpi_definitions, metric_snapshots, dashboards, dashboard_kpis,
 * report_templates, report_schedules, report_runs).
 *
 * Every table is TENANT_FACILITY (dashboards/KPIs/reports are curated per
 * facility; org-wide claims see every facility via the established
 * `OR facility_id IS NULL` claim semantics — DATABASE.md §3.51):
 *
 ...
 * (Phase 1 hardening). Aggregate rows contain no PHI, but they are still
 * tenant-scoped data and are protected like any other domain row.
 *
 * Policy count added: 7 tables × 4 policies = 28 (408 → 436).
 * Scoped matrix: 103 → 110 tables (still 15 off).
 */
return new class extends Migration
{
    /** @var list<string> */
    private const TENANT_FACILITY_TABLES = [
        'kpi_definitions', 'metric_snapshots',
        'dashboards', 'dashboard_kpis',
        'report_templates', 'report_schedules', 'report_runs',
    ];
