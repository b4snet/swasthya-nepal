...
/**
 * Phase 3 slice 2 — RLS for the lab domain (lab_tests, lab_orders,
 * lab_order_items).
 *
 * The three tables are TENANT_FACILITY tier — the exact same policy shape as
 * medications/encounters (SECURITY.md §8, TENANCY.md §6.2):
 *
 ...
 * through the stable helpers (2026_08_13_100200) — the same server-issued
 * context every other policy consumes. Missing/empty claims → NULL → zero
 * access. The runtime role (swasthya_app, NOBYPASSRLS) is bound; FORCE binds
 * the table owner as well (Phase 1 hardening).
 *
 * Policy count added: 3 tables × 4 policies = 12 (144 → 156).
 * Scoped matrix: 37 → 40 tables on (still 15 off).
 */
return new class extends Migration
{
    /** @var list<string> */
    private const TABLES = ['lab_tests', 'lab_orders', 'lab_order_items'];
