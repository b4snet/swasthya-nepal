...
/**
 * Phase 21 — RLS for the CDSS/AI surface (cdss_rules, patient_allergies,
 * cdss_check_results, ai_features, ai_drafts).
 *
 * TENANT_FACILITY tier — the exact same policy shape as the RPM surface
 * (SECURITY.md §8, TENANCY.md §6.2):
 *
 ...
 *
 * Expressions read ONLY the Supabase-compatible `request.jwt.claims` GUC
 * through the stable helpers (2026_08_13_100200). Runtime role swasthya_app
 * (NOBYPASSRLS) bound; FORCE binds the owner as well (Phase 1 hardening).
 *
 * Policy count added: 5 tables × 4 policies = 20 (488 → 508).
 * Scoped matrix: 123 → 128 tables.
 */
return new class extends Migration
{
    /** @var list<string> */
    private const TABLES = ['cdss_rules', 'patient_allergies', 'cdss_check_results', 'ai_features', 'ai_drafts'];
