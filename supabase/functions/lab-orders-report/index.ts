/**
 * lab-orders-report — POST /lab-orders/{id}/report (Phase 4 LIS).
 *
 * THIN DENO ADAPTER. Mirrors LabOrderController::report():
 * - status: verified → reported (CAS on lock_version)
 * - records reported_at
 * - audit: lab.order.reported
 * - Report is immutable after reported (correction requires correction workflow)
 */
import { handleLabOrdersReport } from '../_shared/lab_orders_report.ts';
...
const deps = {
  ...
  reportOrder: (claims: Claims, orderId: string) => {
    ...
    if (order.status !== 'verified') throw new Error('Only verified lab orders can be reported.');

    const updated = db.queryObject<{ id: string }>(
      `update public.lab_orders set status = 'reported', reported_at = now(),
              lock_version = lock_version + 1, updated_by = $1, updated_at = now()
       where id = $2 and tenant_id = $3 and status = 'verified' and lock_version = $4
       returning id`,
    ...
    // Update all items to reported
    db.execute(`update public.lab_order_items set status = 'reported', lock_version = lock_version + 1, updated_by = $1, updated_at = now() where lab_order_id = $2`,
      [claims.app_user_id, orderId]);

    // Update specimens to completed
    db.execute(`update public.specimens set status = 'completed', lock_version = lock_version + 1, updated_by = $1, updated_at = now() where lab_order_id = $2 and tenant_id = $3 and status in ('accessioned', 'processing', 'completed')`,
      [claims.app_user_id, orderId, claims.app_tenant_id]);

    db.execute(`insert into public.audit_events (tenant_id, event_type, auditable_type, auditable_id, payload, created_by) values ($1, 'lab.order.reported', 'lab_order', $2, $3, $4)`,
      [claims.app_tenant_id, orderId, JSON.stringify({ reportedByStaffId: claims.app_user_id }), claims.app_user_id]);

    return { orderId };
  },
};

Deno.serve((req) => handleLabOrdersReport(req, deps));

function postgresFromEnv(): { execute(sql: string, params?: unknown[]): void; queryObject<T>(sql: string, params?: unknown[]): { rows: T[] }; } { throw new Error('lab-orders-report wiring is not importable locally — run it inside Supabase (Deno) with the postgres driver; see supabase/README.md.'); }