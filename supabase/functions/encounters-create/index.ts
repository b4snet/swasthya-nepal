/**
 * encounters-create — ENCOUNTER START / QUEUE HANDOFF (Phase 11), the third
 * write Edge Function.
 *
 * THIN DENO ADAPTER: all logic lives in ../_shared/encounters_create.ts
 ...
 *   2. the appointment lookup and the whole start transaction run as
 *      swasthya_app — RLS is the final boundary (appointments and
 *      encounters are both claims-scoped tables);
 *   3. the ATOMIC start mirrors EncounterController::start: the appointment
 *      status transition is GUARDED (`WHERE id = … AND tenant_id = … AND
 *      facility_id = … AND status = 'checked_in'`) and the encounter INSERT
 *      (tenant/facility/patient/provider all from the appointment) happens
 *      in the SAME transaction. Two concurrent starts of one appointment
 *      yield exactly one success; the loser's guarded update matches zero
 *      rows and rolls the whole transaction back (no partial state, no
 *      orphan encounter). The partial unique index
 *      uq_encounters_tenant_appointment is the fail-closed DB backstop
 *      (one encounter per appointment, DB-enforced);
 ...
 */
import { handleEncountersCreate } from '../_shared/encounters_create.ts';
import type {
  EncounterRow,
  EncounterStartInput,
  EncounterStartResult,
  EncountersCreateDeps,
} from '../_shared/encounters_create.ts';
...
const deps: EncountersCreateDeps = {
  ...identityDeps,
  ...
  startEncounter: (input: EncounterStartInput): EncounterStartResult => {
    ...
    try {
      // GUARDED appointment transition — the DB decides eligibility
      // atomically. A zero-row update (appointment not checked_in — e.g. a
      // concurrent duplicate start already moved it) rolls the WHOLE
      // transaction back: no orphan encounter, no partial mutation.
      const updated = tx.queryObject<{ id: string }>(
        `update public.appointments
            set status = 'in_consultation', lock_version = lock_version + 1, updated_at = now()
          where id = $1 and tenant_id = $2 and facility_id = $3 and status = 'checked_in'
          returning id`,
      ...
      // Encounter INSERT — tenant/facility/patient/provider ALL derived from
      // the RLS-visible appointment (EncounterController::start parity).
      // type=opd, status=open, started_at=now(), lock_version=0.
      const encounter = tx.queryObject<EncounterDbRow>(
        `insert into public.encounters
           (id, tenant_id, facility_id, patient_id, appointment_id, provider_staff_id,
            type, status, started_at, lock_version, created_by)
         values (gen_random_uuid(), $1, $2, $3, $4, $5, 'opd', 'open', now(), 0, $6)
        ...
        [input.tenantId, input.facilityId, input.patientId, input.appointmentId, input.providerStaffId, input.startedBy],
      ).rows[0];
      ...
      if (isUniqueViolation(error)) {
        // Fail-closed backstop: uq_encounters_tenant_appointment (one
        // encounter per appointment). Normally unreachable — the guarded
        // update serializes concurrent starts — but never a 500.
...
Deno.serve((req) => handleEncountersCreate(req, deps));
...
function claimsFor(input: EncounterStartInput): Claims {
  return {
    app_user_id: input.startedBy ?? '',
...
function postgresFromEnv(): {
  ...
  throw new Error(
    'encounters-create wiring is not importable locally — run it inside Supabase (Deno) with the postgres driver; see supabase/README.md.',
  );
}
