/**
 * critical-values-escalate — POST /critical-values/{id}/escalate (Phase 4 LIS).
 *
 * THIN DENO ADAPTER. Mirrors CriticalValueEventController::escalate():
 * - status: triggered → escalated (CAS on lock_version)
 * - records escalated_at, escalated_by_staff_id
 * - audit: lab.critical.escalated
 */
import { handleCriticalValuesEscalate } from '../_shared/critical_values_escalate.ts';
...
const deps = {
  ...
  escalateCriticalValue: (claims: Claims, eventId: string, escalatedByStaffId: string) => {
    ...
    if (event.status !== 'triggered') throw new Error('Only triggered events can be escalated.');

    // CAS transition triggered → escalated
    const updated = db.queryObject<{ id: string }>(
      `update public.critical_value_events set status = 'escalated', escalated_at = now(), escalated_by_staff_id = $1,
    ...
    db.execute(`insert into public.audit_events (tenant_id, event_type, auditable_type, auditable_id, payload, created_by) values ($1, 'lab.critical.escalated', 'critical_value_event', $2, $3, $4)`,
      [claims.app_tenant_id, eventId, JSON.stringify({ escalatedByStaffId: claims.app_user_id, labOrderItemId: event.lab_order_item_id }), claims.app_user_id]);

    return { eventId };
  },
};

Deno.serve((req) => handleCriticalValuesEscalate(req, deps));

function postgresFromEnv(): { execute(sql: string, params?: unknown[]): void; queryObject<T>(sql: string, params?: unknown[]): { rows: T[] }; } { throw new Error('critical-values-escalate wiring is not importable locally — run it inside Supabase (Deno) with the postgres driver; see supabase/README.md.'); }