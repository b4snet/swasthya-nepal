/**
 * encounters-show — the single-encounter READ Edge Function (Phase 20).
 *
 * THIN DENO ADAPTER: all logic lives in ../_shared/encounters_show.ts (pure,
 ...
 *      authorization scope;
 *   3. the presenter shape mirrors EncounterController::present exactly:
 *      the 11 header fields with the three ISO timestamps nullable — NO
 *      related data (the Laravel show contract returns none);
 *   4. the audit row `encounter.viewed` with the exact Laravel payload
 *      {patientId} is appended through the same chain-hashed wiring as all
 *      audit events (AuditLogger parity).
 *
 ...
 */
import { handleEncountersShow } from '../_shared/encounters_show.ts';
import type { EncountersShowDeps, EncounterShowRow } from '../_shared/encounters_show.ts';
...
const deps: EncountersShowDeps = {
  ...
  showEncounter: (claims: Claims, id: string): EncounterShowRow | null => {
    ...
    const row = db.queryObject<EncounterShowDbRow>(
      `select id, facility_id, patient_id, appointment_id, provider_staff_id,
              type, status, started_at, ended_at, signed_at, lock_version
...
Deno.serve((req) => handleEncountersShow(req, deps));

/* ------------------------------------------------------------------ */

interface EncounterShowDbRow {
  id: string;
  facility_id: string;
  patient_id: string;
...
function postgresFromEnv(): {
  ...
  throw new Error(
    'encounters-show wiring is not importable locally — run it inside Supabase (Deno) with the postgres driver; see supabase/README.md.',
  );
}
