/**
 * organizations-locations — the organization-scoped location read Edge
 * Function (Phase 35).
 *
 * THIN DENO ADAPTER: all logic lives in ../_shared/organizations_locations.ts
 ...
 * "validation tiers").
 *
 * The RLS-scoped read is the production-critical wiring (LocationController
 ...
 *   3. the locations SELECT is bound to the VERIFIED organization id and
 *      the claims (locations is TENANT_FACILITY_BRANCH — the select policy
 ...
 *      status filter — active AND inactive locations all return (the
 *      catalog statuses); `facility_id` / `branch_id` are nullable and
 *      HYDRATED by the index select — `facilityId` / `branchId` are
 *      contract-explicit presented fields carrying the real values;
 *   4. NO audit — LocationController::index records no audit event
 ...
 */
import { handleOrganizationsLocations } from '../_shared/organizations_locations.ts';
import type { LocationRow, OrganizationsLocationsDeps, OrganizationLocationsResult } from '../_shared/organizations_locations.ts';
...
const deps: OrganizationsLocationsDeps = {
  ...identityDeps,
  ...
  listOrganizationLocations: (claims: Claims, organizationId: string): OrganizationLocationsResult => {
    ...
    // locations is TENANT_FACILITY_BRANCH — the claims scope the read; the
    ...
    // order). NO status filter — active AND inactive return. Soft-deleted
    // rows are excluded (deleted_at is null). Only the present() columns
    // are selected — tenant/audit/timestamp metadata never leaves the
    // function. `facility_id`/`branch_id` ARE selected and hydrated —
    // `facilityId`/`branchId` carry the real values (unlike the Phase 34
    // branches read where the index select deliberately omits facility_id).
    ...
    const rows = db.queryObject<LocationDbRow>(
      `select id, facility_id, branch_id, name, code, type, status
...
Deno.serve((req) => handleOrganizationsLocations(req, deps));
...
function postgresFromEnv(): {
  ...
  throw new Error(
    'organizations-locations wiring is not importable locally — run it inside Supabase (Deno) with the postgres driver; see supabase/README.md.',
  );
}
