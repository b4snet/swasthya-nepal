/**
 * organizations-wards — the organization-scoped ward read Edge Function
 * (Phase 36).
 *
 * THIN DENO ADAPTER: all logic lives in ../_shared/organizations_wards.ts
 ...
 * "validation tiers").
 *
 * The RLS-scoped read is the production-critical wiring (WardController
 ...
 *   3. the wards SELECT is bound to the VERIFIED organization id and the
 *      claims (wards is TENANT_FACILITY_BRANCH — the select policy is
 ...
 *      status filter — active AND inactive wards all return (the lifecycle
 *      statuses); `facility_id` / `branch_id` are nullable and HYDRATED by
 *      the index select — `facilityId` / `branchId` are contract-explicit
 *      presented fields carrying the real values;
 *   4. NO audit — WardController::index records no audit event
 ...
 */
import { handleOrganizationsWards } from '../_shared/organizations_wards.ts';
import type { OrganizationsWardsDeps, OrganizationWardsResult, WardRow } from '../_shared/organizations_wards.ts';
...
const deps: OrganizationsWardsDeps = {
  ...identityDeps,
  ...
  listOrganizationWards: (claims: Claims, organizationId: string): OrganizationWardsResult => {
    ...
    // wards is TENANT_FACILITY_BRANCH — the claims scope the read; the
    ...
    // `facilityId`/`branchId` carry the real values (the `settings` jsonb
    // is deliberately NOT selected — the Laravel contract does not present
    // it).
    ...
    const rows = db.queryObject<WardDbRow>(
      `select id, facility_id, branch_id, name, code, ward_type, status
         from public.wards
...
Deno.serve((req) => handleOrganizationsWards(req, deps));
...
function postgresFromEnv(): {
  ...
  throw new Error(
    'organizations-wards wiring is not importable locally — run it inside Supabase (Deno) with the postgres driver; see supabase/README.md.',
  );
}
