/**
 * patients-documents — the patient-scoped document-metadata read Edge
 * Function (Phase 32).
 *
 * THIN DENO ADAPTER: all logic lives in ../_shared/patients_documents.ts
 ...
 * "validation tiers").
 *
 * The RLS-scoped read is the production-critical wiring (PatientDocument
 ...
 *      authorization scope;
 *   3. the patient_documents SELECT is bound to the VERIFIED patient id and
 *      the tenant claim (patient_documents is a TENANT_ONLY claims-scoped
 *      table — the patient gate already scoped the caller to this
 *      facility) and ordered by created_at DESC — the exact
 *      `->orderByDesc('created_at')`. NO status filter — staged, available,
 *      archived AND purged documents all return (the lifecycle statuses).
 *      `mime_type`/`size_bytes`/`checksum`/`expires_at`/`retention_class`
 *      are nullable; `uploaded_at`/`expires_at` are ISO timestamp text;
 *      `patientId` is a contract-explicit presented field. The storage
 *      pointer `object_key` is DELIBERATELY NOT selected — the Laravel
 *      contract does not present it (object storage does not exist yet;
 *      records are honestly `staged` with no key) — and there is NO
 *      encrypted field / crypto boundary in this contract;
 *   4. NO audit — PatientDocumentController::index records no audit event
 *      (`patient.document.added` is store-side only).
 *
 ...
 */
import { handlePatientsDocuments } from '../_shared/patients_documents.ts';
import type { PatientDocumentRow, PatientsDocumentsDeps } from '../_shared/patients_documents.ts';
...
const deps: PatientsDocumentsDeps = {
  ...identityDeps,
  ...
  listPatientDocuments: (claims: Claims, id: string): PatientDocumentRow[] | null => {
    ...
    // patient_documents is TENANT_ONLY — the tenant claim + the verified
    // patient binding scope the read; ordered by created_at DESC (the exact
    // Laravel order). NO status filter. The storage pointer `object_key` is
    // deliberately NOT selected (the Laravel contract does not present it —
    // no object storage yet, no crypto boundary).
    const rows = db.queryObject<PatientDocumentDbRow>(
      `select id, patient_id, document_type, mime_type, size_bytes, checksum, status,
...
Deno.serve((req) => handlePatientsDocuments(req, deps));

/* ------------------------------------------------------------------ */

interface PatientDocumentDbRow {
  id: string;
  patient_id: string;
...
function postgresFromEnv(): {
  ...
  throw new Error(
    'patients-documents wiring is not importable locally — run it inside Supabase (Deno) with the postgres driver; see supabase/README.md.',
  );
}
